...
namespace PacMan
{
    ...
    internal class Player
    {
        ...
        public int ChangePosition(KeyboardState input,Map map,int Time, Ghost1 ghost1, Ghost2 ghost2, Ghost3 ghost3, Ghost4 ghost4)
        {
            int dir = GetDirection(input, map);
            if (dir != -1)            
                curDir = dir;
            nextPosition = Position + Dir[curDir];
            if (Time==1)              
                Move(map);
            return Life(ghost1, ghost2, ghost3, ghost4);
        }
        int GetDirection(KeyboardState input, Map map)
        {

            if (input.IsKeyDown(Keys.Up))
            {
                if (map.map[Position.X + Dir[(int)Direction.Up].X, Position.Y + Dir[(int)Direction.Up].Y] != 1)
                {
            ...
            if (input.IsKeyDown(Keys.Down))
            {
                ...
                if (map.map[Position.X + Dir[(int)Direction.Down].X, Position.Y + Dir[(int)Direction.Down].Y] != 1)
                {
            ...
            {
                if (map.map[Position.X + Dir[(int)Direction.Left].X, Position.Y + Dir[(int)Direction.Left].Y] != 1)
                {
            ...
            {
                if (map.map[Position.X + Dir[(int)Direction.Right].X, Position.Y + Dir[(int)Direction.Right].Y] != 1)
                {
        ...
        void Move(Map map)
        {
            ...
            if (map.map[nextPosition.X,nextPosition.Y]!=1)
            {
                ...
                if (map.map[nextPosition.X, nextPosition.Y] == 3) { 
                    map.pills--;                   
                    RageTime = 1;
                    map.map[nextPosition.X, nextPosition.Y] = 0;
                }
        ...
        int Life(Ghost1 ghost1, Ghost2 ghost2, Ghost3 ghost3, Ghost4 ghost4)
        {
            if (Position == ghost1.Position || nextPosition == ghost1.Position || Position == ghost1.nextPosition)
            {
                if (RageTime > 0)
                {
                    return 1;
            ...
            if (Position == ghost2.Position || nextPosition == ghost2.Position || Position == ghost2.nextPosition)
            {
                if (RageTime > 0)
                {                    
                    return 2;
            ...
            if (Position == ghost3.Position || nextPosition == ghost3.Position || Position == ghost3.nextPosition)
            {
                if (RageTime > 0)
                {                    
                    return 3;
            ...
            if (Position == ghost4.Position || nextPosition == ghost4.Position
                || Position == ghost4.nextPosition || nextPosition == ghost4.nextPosition)
            {
                if (RageTime > 0)
                {
                    //Position = nextPosition;
